...
const writingLayers = [
  { key: 'hotspots', no: '01', label: '热点时评', en: 'HOT TOPICS', desc: '按八大知识领域阅读时评文章，点击标题展开正文，像翻“实用技巧”一样一篇一篇学。', items: [] },
...
export function WritingHotspotManual() {
  ...
  const [activeArticle, setActiveArticle] = useState<string | null>('high-quality-development');
  const [activeCaseCategory, setActiveCaseCategory] = useState('people');
  const [activeCase, setActiveCase] = useState<string | null>('huang-wenxiu');
  ...
  const chooseLayer = (key: WritingLayerKey) => { setActiveLayer(key); setDrawerOpen(false); scrollReadingTop(); };
  const chooseCategory = (key: string) => {
    const next = hotspotCategories.find((item) => item.key === key); if (!next) return;
    setActiveLayer('hotspots'); setActiveCategory(key); setActiveArticle(next.articles[0]?.slug ?? null); setDrawerOpen(false); scrollReadingTop();
  };
  const chooseArticle = (slug: string) => { setActiveLayer('hotspots'); setActiveArticle(slug); setDrawerOpen(false); window.setTimeout(() => document.getElementById(`hotspot-${slug}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 40); };
  const chooseCaseCategory = (key: string) => {
    const next = writingCaseCategories.find((item) => item.key === key); if (!next) return;
    setActiveLayer('cases'); setActiveCaseCategory(key); setActiveCase(next.cases[0]?.slug ?? null); setDrawerOpen(false); scrollReadingTop();
  };
  const chooseCase = (slug: string) => { setActiveLayer('cases'); setActiveCase(slug); setDrawerOpen(false); window.setTimeout(() => document.getElementById(`case-${slug}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 40); };
      ...
      <aside className={`framework-manual-sidebar writing-hotspot-sidebar${drawerOpen ? ' open' : ''}`} aria-label="写作积累学习目录">
        ...
        <nav className="framework-layer-nav writing-layer-nav" aria-label="写作积累八类内容">
          {writingLayers.map((item) => {
            ...
            return <div className={`framework-layer-group writing-layer-group${open ? ' open' : ''}`} key={item.key}>
              <button className={`framework-layer-trigger writing-category-trigger${open ? ' active' : ''}`} data-writing-layer={item.key} type="button" aria-expanded={open} onClick={() => chooseLayer(item.key)}><span>{item.no}</span><b>{item.label}</b><i aria-hidden="true">⌄</i></button>

              {open && item.key === 'hotspots' && <div className="framework-layer-children writing-hotspot-domains"><nav className="writing-domain-nav" aria-label="热点时评知识领域">
                {hotspotCategories.map((domain) => {
                  ...
                  return <div className={`writing-domain-group${domainOpen ? ' open' : ''}`} key={domain.key}>
                    <button type="button" className={`writing-domain-trigger${domainOpen ? ' active' : ''}`} aria-expanded={domainOpen} onClick={() => chooseCategory(domain.key)}><span>{domain.no}</span><b>{domain.label}</b><i aria-hidden="true">⌄</i></button>
              ...
              {open && item.key === 'cases' && <div className="framework-layer-children writing-case-domains"><nav className="writing-domain-nav" aria-label="案例素材类型">
                {writingCaseCategories.map((domain) => {
                  const domainOpen = domain.key === activeCaseCategory;
                  return <div className={`writing-domain-group${domainOpen ? ' open' : ''}`} key={domain.key}>
                    <button type="button" className={`writing-domain-trigger${domainOpen ? ' active' : ''}`} aria-expanded={domainOpen} onClick={() => chooseCaseCategory(domain.key)}><span>{domain.no}</span><b>{domain.label}</b><i aria-hidden="true">⌄</i></button>
              ...
              {open && item.key !== 'hotspots' && item.key !== 'cases' && <div className="writing-topic-preview writing-layer-preview" aria-label={`${item.label}内容索引`}>{item.items.map((topic) => <span key={topic}>{topic}</span>)}</div>}
            </div>;
          })}
        </nav>
      </aside>
          ...
          <div className="tips-accordion writing-learning-accordion">
            {category.articles.map((entry) => {
              ...
              return <section className={`tips-article writing-learning-item${open ? ' open' : ''}`} id={`hotspot-${entry.slug}`} key={entry.slug}>
                <button className="tips-article-trigger" type="button" aria-expanded={open} onClick={() => setActiveArticle(open ? null : entry.slug)}>
          ...
          <div className="tips-accordion writing-learning-accordion writing-case-accordion">
            {caseCategory.cases.map((entry) => {
              ...
              return <section className={`tips-article writing-learning-item${open ? ' open' : ''}`} id={`case-${entry.slug}`} key={entry.slug}>
                <button className="tips-article-trigger" type="button" aria-expanded={open} onClick={() => setActiveCase(open ? null : entry.slug)}>
                  <span className="tips-article-no">{entry.no}</span>
