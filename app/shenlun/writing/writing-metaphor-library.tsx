...
export function WritingMetaphorLibrary() {
  ...
  const filtered = useMemo(() => {
    if (!keyword) return metaphorEntries.map((entry, index) => ({ entry, index }));
    return metaphorEntries
  ...
  return (
    <section className={styles.library}>
      <header className={styles.intro}>
        ...
        <div className={styles.notice}><b>{metaphorEntries.length} 条</b><span>权威语料筛选 + 教学释义整理</span><em>不要求按顺序背，直接搜索需要的表达</em></div>
      </header>
      ...
      <div className={styles.list}>
        {filtered.map(({ entry, index }) => {
          ...
          return <section className={`${styles.item}${open ? ` ${styles.open}` : ''}`} id={`metaphor-term-${index}`} key={entry.term}>
      ...
      <footer className={styles.sources}>
        ...
        <div>{metaphorSourceLinks.map((source) => <a href={source.href} target="_blank" rel="noreferrer" key={source.href}>{source.label}<i>↗</i></a>)}</div>
      </footer>
    </section>
  );
}
